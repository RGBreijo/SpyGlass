            ...
            this.state.goals.map(goal => {
                    return (
                        <div className="goalCard" key={goal.goalId}>
                            ...
                            <div>
                                <ProgressBar
                                    completed={((goal.initialDeposit + goal.monthlyDepo) * 100) / goal.targetAmount}
                                    transitionDuration="1.5s" bgColor="#04D582" borderRadius="5px" baseBgColor="#E3F3FD"
                                    isLabelVisible={false} height="15px"/>
                            </div>
                            <div className="priceContainer">
                                <CountUp prefix="$" isCounting thousandsSeparator="," start={0}
                                         end={(goal.targetAmount - goal.initialDeposit)}
